...
| **[vaastav/Fantasy-Premier-League](https://github.com/vaastav/Fantasy-Premier-League)** | Historical archive back to 2016/17 | Good for backtesting/training projection models. Understat data lives in the same repo but is **not pre-merged** — see the Understat row below for the real linkage limits (verified directly against live files, not assumed). |
| **Understat** | Raw xG, xA, xGI, xGC | Reached via vaastav's repo (`data/<season>/understat/`), not scraped directly. **Team-level** match data is broad (seasons 2019-20 through 2024-25, confirmed by fetching every season's file listing — absent 2016-17→2018-19 and 2025-26). **Player-level** linkage to FPL identity depends on that season's `id_dict.csv`, which — verified file-by-file, not assumed from the README — exists for only **2021-22 and 2022-23**, not an ongoing range. Outside those two seasons, player-level Understat rows are stored unlinked (real data, no FPL player_code) rather than dropped or fuzzy-matched by name. |
